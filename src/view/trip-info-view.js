...
import { humanizePointEventDate } from '../utils/point.js';
...
const createTripInfoTemplate = (points, offers) => {
  ...
  const starDate = humanizePointEventDate(points[0].dateFrom);
  const finishDate = humanizePointEventDate(points[points.length-1].dateTo);
